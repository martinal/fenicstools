...
# Hack to make it possible to use some functionality here without pyvtk installed
try:
    import pyvtk
except:
    pyvtk = None

# Hack to make it possible to use some functionality here without h5py installed
try:
    import h5py
except:
    h5py = None
...
def _compile_module():
    source_folder = os.path.abspath(os.path.join(inspect.getfile(inspect.currentframe()), "../Probe"))
    sources = ["Probe.cpp", "Probes.cpp", "StatisticsProbe.cpp", "StatisticsProbes.cpp"]
    headers = ['Probe.h', 'Probes.h', 'StatisticsProbe.h', 'StatisticsProbes.h']   
    headers = map(lambda x: os.path.join(source_folder, x), headers)
    ...
    compiled_module = compile_extension_module(code=code,
        ...
        sources=sources,
        include_dirs=[".", os.path.abspath(source_folder)])
    return compiled_module

# Execute compilation on import of this module, this is
# necessary because classes below inherit from compiled classes
compiled_module = _compile_module()
